...
{  char *Name ;             // имя формата
   ...
   short  find_struct( char Dst, char *Tag );
   short  find_struct_name( char Dst, char *Name );
...
{  e_Type type ;       // слово,структура или выбор
   ...
   short  order ;      // индекс упорядочивания (числовой)
   char  *s_order ;    // индекс упорядочивания (строковый)
   short  i_struct ;   // индекс структуры в массиве t_All_Data->Trans
   char   use ;
   char   grammar_use ; // главное грамматическое слово
...
// ============= жизнь внутри слов =============================
// ------------- соответствие в файле semantika.txt -------------
struct t_semLink
{
   short type_link ; // тип связки
   char  *Name ;     // имя
   
         t_semLink();
   void  init();
};
DECLARE_LIST_TYPE( t_semLink, t_semLinkList );

// ------------- откуда куда шагать ----------------------------
struct t_Shift1
{
   char   *Prefix ;   //  приставка
   char   *Suffix ;   //  суффикс
   short   i_struct ; //  структура
   t_Form  Form ;     //  форма

         t_Shift1();
   void  init();
};
DECLARE_LIST_TYPE( t_Shift1,t_Shift1List );

// ----------- "типичное" склонение слов -----------------------
// -----------  смысловой сдвиг --------------------------------
struct t_Shift
{
   char         *Name ;     // название шага
   long          i_str ;    // строка, в которой содержится описание
   t_Shift1      From,To ;  // Из чего во что происходит преобразование
   t_Trans       Trans ;    // Аналитический способ преобразования
   t_semLinkList sLink ;    // связкаи, которые соответствуют файлу семантики
   t_strList     str ;      // окончания склонения (в соответствии с форматом)
   short         i_struct_from,
                 i_struct_to ;
   ...
   void  init( void );
   void  del( void );
   t_Shift & operator = ( t_Shift &SS );
};
DECLARE_LIST_TYPE( t_Shift, t_ShiftList );

// ------------- типы связок ---------------------------------
struct t_TypeLink
{
   char  *Name ;     // имя типа связки
   char  *remark ;   // коментарий
};
DECLARE_LIST_TYPE( t_TypeLink, t_TypeLinkList );

// -------------- формат, описывающий семантический словарь -------
struct t_Format4
{  char *Name ;               // имя формата
   t_PartDskList   Part ;     // список переименования структур
   t_RenameList    Rename ;   // список переименования параметров (пока не используется)
...
DECLARE_LIST_TYPE( t_Format4, t_Format4List );
...
struct t_Lang     
{
   ...
   t_Format4List Format4 ;// форматы семантических словарей
   t_FileList    File ;   // файлы грамматических словарей
   t_FileList    SemFile ;// файлы сементических словарей
   t_ShiftList   Ends ;   // типичное словоизменение
   t_Lexer      *Lexer ;
   ...
   long  read_format4( long Begin, t_Format4 *Format );
//   void  read_end( long Begin, t_Shift &End1 );
   short quest(  short i_struct, char *Str, long *Ant, t_Form *Form );
   char *normal( short i_struct, char *Str );
   char *form(   short i_struct, char *Str, t_Form *Form1 );
};
...
//   t_Format1List  Format1 ;  // форматы переводов
   t_Format1      Format1 ;  // формат переводов
   ...
   t_FileList     FileSemantika ; // имена файлов смысловых связей (приемника)
public:
   t_FileList     File2 ;    // имена файлов словарей для человека
private:
   t_TransList     Trans ;    // список всех трансляционных пар 
public:
   t_ShiftList     Step ;     // список шагов словообразования
   t_AlgorithmList Algorithm ;// список алгоритмов словообразования
private:
   ...
   short          i_Unknown_s ; // "@знаковые_части_речи" источника
   short          i_Unknown_d ; // "@знаковые_части_речи" приемника
   short          i_Wisard ;  // корень визарда фраз
public:
   char           f_comma ;   // учитывать запятые при переводе
   short          i_Algorithm ; // индекс главного алгоритма словообразования
...
private:   
   ...
   long    read_perevodik( long Begin );
//   long    read_step ( long Begin );
   long    read_step1( long Begin, t_Shift1 &Part );
   long    read_algorithm ( long Begin );
   long    read_algorithm1( t_Algorithm *Al, long Begin );
