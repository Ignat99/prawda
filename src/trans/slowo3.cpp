...
static t_String StrErr ;  // строка ошибки (как-то это не по плюсовому)
...
{
   fprintf( File_Error,"\n Ребята, че-то я словарь %s не могу прочесть.",FileName1.c_str() );
   ...
   // ------ проверить TSTRUCT1 ----------------------
{
   ...
   if( f_error==1 ) throw(-1); // эта ошибка не перехватывается
}
...
catch( int E )
{
   ...
   Read_Error=-1 ;
   throw( E ); // эта ошибка не перехватывается
}
}
/************************************************************************/
// определить является ли запись, начинающаяся с Mass+z_begin структурой
/************************************************************************/
   ...
   {  c=Str[i] ;
	  ...
	  else
	  {
		...
		throw(-1);
	     ...
	     if( i1<0 )
	     {  StrErr="make_relation2::параметр структуры не найден" ;
	        error_set( StrErr.c_str() );
	        throw(-1);
	     }
...
void t_Slowo3 :: make_tree( void )
{
   ...
   Tree[j-1].last=n_Record-1 ; // n_Record ; (2 января 2005г)
   Tree[0].n_down=j-1 ;
...
/***************************************************************************/
//         проверить корректность перевода (для контекст-2)
/***************************************************************************/
...
{  short i,f1,f2,f3 ;   
   ...
   try
   {
      ...
      for( i=0 ; Str[i]!=0 ; i++ )
         if( Str[i]=='=' ) goto M1;
      return "В строке нет знака равно" ;
M1:   for( i=0 ; Str[i]!='=' ; i++ )
      {  if( Str[i]==':' ) f_struct1++ ;
         if( Str[i]=='[' ) goto M2;
      }
      return "В строке нет квадратных скобок (до знака равно)" ;
M2:   for( i=0 ; Str[i]!=0 ; i++ )
      {  if( Str[i]==':' ) f_struct2++ ;
         if( Str[i]=='[' ) goto M3;
      }
      return "В строке нет квадратных скобок (после знака равно)" ;
M3:   f1=f2=f3=0 ;
      for( i=0 ; Str[i]!=0 ; i++ )
      {  c=Str[i] ;
	 if( '['==c ) f1++ ;
	 ...
	 if( '='==c && ( f1!=0 || f2!=0 || f3!=0) )
      ...
      if( f1!=0 || f2!=0 || f3!=0 )
	  return "Ошибка непарности скобок" ;
      if( 1<f_struct1 || 1<f_struct2 )
	  return "Ой! А зачем вы так много двоеточий поставили?" ;
      if( f_struct1!=f_struct2 )
	  return "Слово должно переводиться словом, структура - структурой" ;
 ...
 /*
	  ...
	  return "Несоответствие части речи оригинала и перевода" ;
...
void t_Slowo3 :: s_universe( short i_word, short i_struct )
{
   ...
   {  i_tree=Node[i].link ;
	 ...
	 for( i1=0 ; i1<NN; i1++ )	 
	 {  // ------- цикл по вариантам -------------
	    if( 0==W->str[0] ) // только что внес
	             Str="" ;
	        else Str=Core.get_meaning( &VV->Variant[i1] ) ; // смысл варианта
	    find_bin( Str, i_tree );
...
//        make_relation2( fdst, S1, iw, Reg, j_Relation ); // пишет в Relation
//     3) Использование Grammar->Format
/************************************************************************/
...
try
{
   ...
   fformat=fstr=is_struct( Mass ); // имеет побочные эффекты
   ...
   for( i1=0,j=0 ; Mass[i1]!=0 && Mass[i1]!='\n' ; i1++ )
   {
	 ...
	 continue ;
	  ...
	  {   short i_str ;
		  ...
		  if( i_str<=-10 )
        	  {   StrErr="Неизвестный тэг\"" ; StrErr+=Reg ; StrErr+="\"" ; throw(-1);  }
		  S.i_struct=i_str ;
