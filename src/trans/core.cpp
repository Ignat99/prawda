...
t_Slowo3     Macro(-1) ;    // словарь английских сокращений (препроцессор)
t_Slowo3     Micro(-1) ;    // словарь русских сокращений (постпроцессор)
t_Slowo3     DicConst(-1) ; // словарь английских constant
t_Slowo3     Perevod1(0) ;   // словарь переводов
t_Slowo3     Perevod2(0) ;   // словарь переводов (контекстный)
t_Slowo3     Dogadka(1) ;   // словарь догадок

t_Slowo3    *Perevod=&Perevod1 ;

t_Semantika  Semantika ; // словарь смысловых связей (приемника)
...
short find_enum_select( e_Half H, short up_struct, long i_slowo, char i_dic );
...
void  clear_str( char *Str, char *Str1, short Lmax );

// ------ обертки для унификации Perevod и Dogadka -----------------
t_Form      P_word_param( char i_dic, long i_slowo, long i_variant, long i_word );
t_Form      P_struct_param( char i_dic, long i_slowo, long i_variant=0 );
short       P_get_n_perevod( char i_dic, long i_slowo );
void        P_find( char i_dic, char *From, short i_struct, t_longList *Antwort );
short       P_part( char i_dic, char *From, short *i_Part );
t_sStruct  *P_get_from( char i_dic, long i_slowo );
t_sStruct  *P_get_to( char i_dic, long i_slowo, long i_slowo1=0 );
char        P_is_atom( char i_dic, long i_record );
char       *P_translate_word_i( char i_dic, char *From, long  i_slowo,  short i_slowo1=0 );
char       *P_translate_word_s( char i_dic, char *From, short i_struct, short i_slowo1=0 );
char       *P_word_src1( char i_dic, long i_slowo );
t_RelationList1  P_get_relation( char i_dic, long i_slowo, long i_variant );
...
Perevod=&Perevod1 ; // по умолчанию перевод по главному словарю
...
try
{
   ...
   // -------------- чтение словарей пре- и пост- процессора ---------
   {
	  ...
	  if( 0<File1[0].FileName.j )
	      Macro.read( filename( Path,File1[0].FileName[0] ), &Grammar.format1() );

      t_FileList &File2=Grammar.file_post() ;
      Micro.del( );
      if( 0<File2.j )
	  if( 0<File2[0].FileName.j )
	      Micro.read( filename( Path,File2[0].FileName[0] ), &Grammar.format1() );
   }
   ...
   // ---------------- прочитать словари семантики приемника ---------
   if( 0<To.SemFile.j && 0<To.SemFile[0].FileName.j )
   {  t_File *SF= &To.SemFile[0] ;
      Semantika.read( filename( Path,SF->FileName[0]), &To.Format4[SF->i_format] );
      Semantika.reverse();
   }
   // -------------- прочитать словари перевода ---------------
   Perevod1.read( Path, &Grammar );
   Dogadka.set_format( Perevod1.format() ) ;
   Fclose( File_Error );
...
{  t_Struct *S ;
   ...
   DicConst.make( Mass, &Grammar.format1() );
   Free( Mass );
}
/***************************************************************************/
...
/***************************************************************************/
//      установить словарь перевода - "главный" или "контекстный"
/***************************************************************************/
void set_main_dict( char f_kontext )
{
   if( 0==f_kontext ) Perevod=&Perevod1 ;
       else           Perevod=&Perevod2 ;
}
/***************************************************************************/
t_Core :: t_Core( )
{
...
/***************************************************************************/
//   Разбить фразу на слова (нужна для построения контекстного словаря)
//   Sou - строка источника
/***************************************************************************/
void t_Core :: translate_from( char *_Sou )
{
   long i ;
try
{
...
char t_Core :: make_from1( char *Source )
{
   ...
   strcpy( Sou1,Source );
   // ------- определить завершающий символ фразы ------------
   Sign='.' ;
   for( i=strlen( Sou1 ); 0<i ; i-- )
   {  c=Sou1[i] ;
      if( c=='.' || c=='!' || c=='?' || c==':' ) 
      {   Sign=c ; break ;  }
   }
   // ------- отделить знаки препинания от предшествующих слов ------------
   t_String S ;
   //S.reserve(300);
   S="" ;
   for( i=0 ; Sou1[i]!=0 ; i++ )
   {  c=Sou1[i] ;
      if( c=='.' || c=='!' || c=='?' || c==':' )
      {   continue ; // убрать завершающую точку (пока путь так)
          if( 0<i && ' '!=Sou1[i-1] )
              S+=' ' ;
      }
      if( ','==c )
      {   if( 1==Grammar.f_comma )
          {   S+=' ' ; S+=c ;  }
          continue ;
      }
      S+=c ;
   ...
   L=strlen( Sou1 );
   for( i=f=j=0 ; i<L ; i++ )
   {                          // где-то в этом цикле надо отделять запятые
      c=Sou1[i] ;
      if( c==' ' || c=='\t' || c=='\n' )
      {   if( 0<strlen(S.c_str()) )
          {   
	      strcat( Sou0, Macro.translate_word_s( S.c_str(),-1 ) );
	      strcat( Sou0," " );
	      S="" ;
	  }
      }
...
char t_Core :: make_from2( void )
{
   ...
   {  From[i_word].i_slowo=-1 ;
      
      clear_str( From[i_word].Str,Str2,100 );
      Str=Str2 ;
      
      if( is_number(Str) )
      {  // -------- проверить, что это числительное -------
	 add_from( i_word, Grammar.i_number(), t_Form(), 0, -1, -1 );
	 continue ;
      }
	    ...
	    {  t_Form        FormPP=Perevod->word_param( Ant2[i3],-1,0 );
		  ...
		  if( Param->list[i4].Dir==0 )
	       ...
	       N1=Perevod->get_n_perevod( Ant2[i3] );
  	       add_from( i_word, i_part, FormPP, 0, Ant2[i3], N1 );
	       From[i_word].i_slowo=Ant2[i3] ;
            }
	    From[i_word].exist=1 ;
	 }
      }
         ...
         for( i3=0 ; i3<Ant2.j ; i3++ )
	 {
	    ...
	    N1=Perevod->get_n_perevod( Ant2[i3] );
            add_from( i_word, part[i2], FormPP, 0, Ant2[i3], N1 );
      Now_Exist	:
            From[i_word].i_slowo=Ant2[i3] ;
	    From[i_word].exist=1 ;
	 }
	 ...
	 for( i1=0 ; i1<Grammar.from().File.j ; i1++ )
	 {  i_part=SlowoF[i1].Format->i_part ;
	    N   =Grammar.from().quest( i_part,Str, Ant1, Form );
	    for( i2=0 ; i2<N ; i2++ )
	    {  Str1=Grammar.from().normal( i_part,Str );
               // ------ кусок (12 строк) АБСОЛЮТНО ОДИНАКОВЫЙ с предыдущим --------
	       Perevod->find( Str1,i_part,&Ant2 );
	       if( 0<Ant2.j )
               {  // ------ исходная форма есть в главном словаре
        	  for( i3=0 ; i3<Ant2.j ; i3++ )
		  {  t_Form        FormPP=Perevod->word_param( Ant2[i3],-1,0 );
		     t_Param1List *Param =&Grammar[i_part].From.Param ;
		     for( i4=0 ; i4<Param->j ; i4++ )
	       ...
	       {  // ------ слова нет в главном словаре (догадаться по родственным словам)
        	  t_longList Ant3 ;
		  
		  Core1.make_dogadka( Str1,Ant3 );
		  if( 0<Ant3.j ) // ----- если аналитический перевод, то ответ равен нулю !!!
        	  {   for( i3=0 ; i3<Ant3.j ; i3++ )
		      {  t_Form        FormPP=Dogadka.word_param( Ant3[i3],-1,0 );
			 t_Param1List *Param =&Grammar[i_part].From.Param ;
			 for( i4=0 ; i4<Param->j ; i4++ )
			    if( Param->list[i4].Dir==0 )
	        		FormPP.value[i4]=Form[i2].value[i4] ;

			 N1=Dogadka.get_n_perevod( Ant3[i3] );
			 short i_part1=Dogadka.get_from( Ant3[i3] )->i_struct ;
  			 add_from( i_word, i_part1, FormPP, 1, Ant3[i3], N1 );
        	      }
		      From[i_word].exist=1 ;
	          }
	       }
...
void t_Core :: add_from( short i_word, long i_struct, t_Form Form, 
               char i_dic, long i_slowo, short n_slowo )
{  t_Variants *VV ;
   t_rStruct   S ;
   t_rWord     W ;
   ...
   if( TSTRUCT1==S.type ) // ---- структура из словаря 
   {                      // ---- сделан случай только с одним словом !!!
      t_sStruct   *S1 ;
      if( i_dic==0 ) S1=Perevod->get_from( i_slowo );
      if( i_dic==1 ) S1=Dogadka.get_from( i_slowo );
       
      W.i_struct = S1->Word[0].i_struct ;
   }   
   
   if( 0<=i_slowo )
       set_meaning( &VV->Variant[VV->Variant.j-1], P_word_src1( i_dic, i_slowo ) );
}
/**************************************************************************/
...
void t_Core :: universe( short i_word, long i_struct )
{
   ...
   {   // ---- структура из словаря -----
       ...
       Dogadka.s_universe( i_word, i_struct );
       sp-- ;
       return ;
   }
   ...
   {  t_TreeList Node ;
	 ...
	 {  pN1=&Node[z] ;
	    if( pN1->i_struct==Grammar.i_irregular() && 0<=Grammar.i_irregular() )
	        continue ;
	 ...
	 if( 0<z && Node[z].i_struct==Grammar.i_irregular() && 0<=Grammar.i_irregular() )
...
char t_Core :: atom( short i_word, t_Word &Word )
{
   ...
   {  // ---------------- если это слово ----------
      ...
      {  // ----слово отсутствует в словаре - сделать вид, что оно подходит --
         if( 0<=Grammar.i_unknown(0) )
	 {   t_Struct *S=&Grammar[Grammar.i_unknown(0)].From ;
   ...
   M_Ok: 
         if( VV->Variant.j<1 ) // защита от повторного внесения
             add_from( i_word, Word.i_struct, t_Form(), 0, -1, -1 );
	 return 1 ;
      }
...
void t_Core :: real_param_up( t_rStruct *V )
{
   ...
   if( SS->type==TSTRUCT2 )
        RR=P_get_relation( V->i_dic, V->i_slowo, -1 );
...
t_Form  P_word_param( char i_dic, long i_slowo, long i_variant, long i_word )
{
   if( i_dic==0 ) return Perevod->word_param( i_slowo, i_variant, i_word );
       else       return Dogadka.word_param( i_slowo, i_variant, i_word );
}
/***************************************************************************/
t_Form  P_struct_param( char i_dic, long i_slowo, long i_variant )
{
   if( i_dic==0 ) return Perevod->struct_param( i_slowo, i_variant );
       else       return Dogadka.struct_param( i_slowo, i_variant );
}
/***************************************************************************/
short  P_get_n_perevod( char i_dic, long i_slowo )
{
   if( i_dic==0 ) return Perevod->get_n_perevod( i_slowo );
       else       return Dogadka.get_n_perevod( i_slowo );
}
/***************************************************************************/
void   P_find( char i_dic, char *From, short i_struct, t_longList *Antwort )
{
   if( i_dic==0 ) Perevod->find( From, i_struct, Antwort );
       else       Dogadka.find( From, i_struct, Antwort );
}
/***************************************************************************/
short  P_part( char i_dic, char *From, short *i_Part )
{
   if( i_dic==0 ) return Perevod->part( From, i_Part );
       else       return Dogadka.part( From, i_Part );
}
/***************************************************************************/
t_sStruct *P_get_from( char i_dic, long i_slowo )
{
   if( i_dic==0 ) return Perevod->get_from( i_slowo );
       else       return Dogadka.get_from( i_slowo );
}
/***************************************************************************/
t_sStruct *P_get_to( char i_dic, long i_slowo, long i_slowo1 )
{
   if( i_dic==0 ) return Perevod->get_to( i_slowo, i_slowo1 );
       else       return Dogadka.get_to( i_slowo, i_slowo1 );
}
/***************************************************************************/
t_RelationList1  P_get_relation( char i_dic, long i_slowo, long i_variant )
{
   if( i_dic==0 ) return Perevod->get_relation( i_slowo, i_variant );
       else       return Dogadka.get_relation( i_slowo, i_variant );
}
/***************************************************************************/
char   P_is_atom( char i_dic, long i_record )
{
   if( i_dic==0 ) return Perevod->is_atom( i_record );
       else       return Dogadka.is_atom( i_record );
}
/***************************************************************************/
char *P_translate_word_i( char i_dic, char *From, long  i_slowo, short i_slowo1 )
{
   if( i_dic==0 ) return Perevod->translate_word_i( From, i_slowo, i_slowo1 );
       else       return Dogadka.translate_word_i( From, i_slowo, i_slowo1 );
}
/***************************************************************************/
char *P_translate_word_s( char i_dic, char *From, short i_struct, short i_slowo1 )
{
   if( i_dic==0 ) return Perevod->translate_word_s( From, i_struct, i_slowo1 );
       else       return Dogadka.translate_word_s( From, i_struct, i_slowo1 );
}
/***************************************************************************/
char *P_word_src1( char i_dic, long i_slowo )
{
   if( i_dic==0 ) return Perevod->word_src1( i_slowo );
       else       return Dogadka.word_src1( i_slowo );
}
/***************************************************************************/
//              типа интерфейс защитного программирования
/***************************************************************************/
...
void t_Antwort :: make_sou( short i_word, short i_struct, long i_variant )
{
   ...
   for( i=0 ; i<Tree1.j ; i++ )
   {
	  ...
	  if( t==TSTRUCT2 )
	  {   // ------ структура из словаря ------
	      sS=P_get_from( pS->i_dic, pS->i_slowo );
	      pT->i_slowo     = pS->i_slowo ;
	      pT->n_down      = sS->n_Word ;
...
{  long       i,i1,i2,is,n ;
   ...
   {   S2=P_get_to( Tree1[0].i_dic, Tree1[0].i_slowo,Tree1[0].i_slowo1 );
   ...
   {  T2=&Tree2[i] ;
      ...
      {
	  S2=P_get_to( T2->i_dic, T2->i_slowo,T2->i_slowo1 );
	  T2->n_down   =S2->n_Word ;
      ...
      if( t==TSELECT1 )
      {   T2->n_down   =1 ;
	  T2->select=find_enum_select( TO, T2->i_struct, T2->i_slowo, T2->i_dic );
...
void t_Antwort :: make_dst1( long up, long i2, t_Trans *Tr )
{
   ...
   {   // -------- родительская структура из словаря ---------------
       ...
       S1=P_get_from( Tree2[up].i_dic, i3 );
       S2=P_get_to(   Tree2[up].i_dic, i3,Tree1[i_link].i_slowo1 );
...
{  long       i ;
   ...
   {   if( T->link<0 ) return ;
       ...
       Str2=P_translate_word_i( T1->i_dic, Str1, T1->i_slowo, T1->i_slowo1 );       
       if( Str2!=NULL )
   ...
   {   if( T->type==TSTRUCT || T->type==TUNORDER )
       ...
       {   // ---- структура из словаря --------------------------
	   S=P_get_to( T->i_dic, T->i_slowo,T->i_slowo1 );
...
{  short     i_slowar,i_struct,z_begin,N ;
   ...
   if( 0<=i_slowar && Grammar[i_struct].From.type==TWORD )
   ...
   Str0=P_translate_word_s( 0, Str1, i_struct ); // i_dic можно протащить через From
   Str+=Str0 ;
   Str+=" " ;
   return Str.c_str()+z_begin ;
}
...
void t_Antwort :: param_from( void )
{
   ...
   {  F=&Tree1[i] ;
      ...
      if( F->type==TSTRUCT2 )
              RR=P_get_relation( F->i_dic, F->i_slowo, -1 );
...
void t_Antwort :: param_to( void )
{
   ...
   {  T=&Tree2[i] ;
      if( T->type==TSTRUCT1 || T->type==TSTRUCT2 )
          T->Form=P_struct_param( T->i_dic, T->i_slowo, T->i_slowo1 );
   }
   // ------------- проход снизу вверх ------------------
   for( i=Tree2.j-1 ; 0<=i ; i-- )
   {
      ...
      SS=&Grammar[T->i_struct].To ;
      if( T->type==TSTRUCT2 )
               RR=P_get_relation( T->i_dic, T->i_slowo, T->i_slowo1 );
	  else RR=SS->Relation ;

      // --------- вычисление постоянных параметров (слов) ----
      if( T->type==TWORD )
      {   if( 0<=T->up )
              T_up=&Tree2[T->up] ;
	  if( 0<=T->up && (T_up->type==TSTRUCT1 || T_up->type==TSTRUCT2) )
		   Form=P_word_param( T_up->i_dic, T_up->i_slowo, T_up->i_slowo1 , T->index );
	      else Form=P_word_param( T->i_dic   , T->i_slowo   , T->i_slowo1    ,0 );

	  for( i1=0 ; i1<SS->Param.j ; i1++ )
	     if( SS->Param[i1].Dir==1 )
		 T->Form.value[i1]=Form.value[i1] ;
      }
      // --------- вычисление постоянных параметров (структур) ----
      if( IF_STRUCT( T->type ) )
      {
	  for( i1=0 ; i1<SS->Param.j ; i1++ )
	     if( 0<=SS->Param[i1].value )
		 T->Form.value[i1]=SS->Param[i1].value ;
      }
   ...
   for( i=0 ; i<Tree2.j ; i++ )
   {
	 ...
	 T2->Form.value[R.p2] = T1->Form.value[R.p1] ;
      ...
      {   // --------- и для структур ----------
          t_Form F ;
          if( T->type==TSTRUCT1 ) 
	  {  sS=P_get_to( T->i_dic, T->i_slowo );
	     //  F=sS->Word[i1].Param.form();        // ОШИБКА!!! i1 в этом месте не присвоено!!!
	  }
	  for( i1=0 ; i1<SS->Word.j ; i1++ )
	  {
	     for( i2=0 ; i2<SS->Word[i1].Param.j ; i2++ )
		...
		if( 0<=v )
		    Tree2[T->down+i1].Form.value[i2]=v ;
                */
		// изменял когда правил аналитическое словообразование
		v=SS->Word[i1].Param[i2].value ;
		if( v<=0 && T->type==TSTRUCT1 && SS->Word[i1].Param[i2].Dir==1 )
...
//              именно здесь происходит склонение приемника
...
{  t_Tree    *T ;
   ...
   if( T->type==TWORD )
   {
      ...
      {  char *s ;
         ...
         // вот тут надо использовать шаблон склонения из grammar.txt
	 if( 0>Slowo->word_exist( T->Str1 ) )
	          s=Grammar.to().form( T->i_struct,T->Str1,&T->Form );
	     else s=Slowo->form( T->Str1,&T->Form);
         Strcpy( T->Str, s,LWORD );
   ...
   {   // ---- возможно, что тут еще надо теги вставлять
	  ...
	  print_antwort( T->down+i,fword );
       ...
       if( T->type==TSTRUCT1 || T->type==TSTRUCT2 )
          if( 0==P_is_atom( T->i_dic, T->i_slowo) )
	  {   add_word( *T,fword );
          }
   }
   if( IF_SELECT( T->type ) )
   {  short z_str =Str.length( );
      short z_word=aWord.j ;
      print_antwort( T->down,fword && (T->type!=TSELECT2 ) );
      if( T->type==TSELECT2 && fword )
      {   add_word( *T,1 );
          if( z_word<aWord.j ) // сам не знаю зачем, но иначе падает
              Strcpy( aWord[z_word].Str,Str.c_str()+z_str,LWORD );
      }
   }
   sp-- ;
}
...
void t_Antwort :: add_word( t_Tree &T, char fword )
{
   ...
   if( (T_up->type==TSTRUCT1 || T_up->type==TSTRUCT2) &&
          0==P_is_atom( T_up->i_dic, T_up->i_slowo ) ) return ;
   if( fword==0 ) return ;
   ...
   W.i_slowo     = T.i_slowo ;
   if( 0<=T.i_slowo ) W.n_slowo = P_get_n_perevod( T.i_dic, T.i_slowo );
       else           W.n_slowo = 1 ;
   W.Form        = T.Form ;
   W.Str[0]      = 0 ;
   if( IF_WORD( T.type ) )
...
void t_Antwort :: post_proc( void )
{
    ...
    c=' ' ;
    L=Str.length();
    for( i=j=0 ; i<LFRASA && i<L ; i++ )
    {  if( !(Str[i]!=' ' && c==' ' ) ) goto M_Normal ;
       Micro.find( Str.c_str()+i,-1,&Ant );
...
{  t_Struct  *S0 ;
   ...
   {  S0=&Grammar[up_struct].From ;
      if( 0==i_dic ) S1=Perevod->get_from( i_slowo );
          else       S1=Dogadka.get_from( i_slowo );
   }
   else
   {  S0=&Grammar[up_struct].To ;
      if( 0==i_dic ) S1=Perevod->get_to( i_slowo );
          else       S1=Dogadka.get_to( i_slowo );
   }
...
/***************************************************************************/
//                    очистить строчку от "грязи"
/***************************************************************************/
void clear_str( char *Str, char *Str1, short Lmax )
{  short i,j ;

   for( i=0 ; Str[i]!=0 ; i++ )
      if( 0==is_gras( Str[i] ) ) break ;
   for( j=0 ; Str[i]!=0 ; i++ )
   {  if( Lmax<=j-1 ) break ;
      if( 1==is_gras( Str[i] ) ) break ;
      Str1[j++]=Str[i] ;
   }
   Str1[j]=0 ;
}
