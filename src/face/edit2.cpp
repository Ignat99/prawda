...
//
//  t_FirstLook   отображение первичного массива слов
...
/***************************************************************************/
t_FirstLook :: t_FirstLook( void ) : t_Win( )
{
   restart();
}
/***************************************************************************/
void t_FirstLook :: restart( void )
{
   Begin   = 0 ;
   Begin_X = 0 ;
   PY      = 0 ;
...
/***************************************************************************/
void t_FirstLook :: paint( void )
{  short i ;
   char  Str[20] ;

   t_Win::paint();
   sprintf( Str,"(%2d/%2d)",PY,MaxY );
   s_text_yxf( y1,x2-10 ,0x07, Str );
...
/***************************************************************************/
void t_FirstLook :: paint1( short i, char current )
{  short p1,p2,p3 ;
   short i_struct ;
   char  c ;
   char  *Str,*Str1,Str2[100] ;
   ...
   {   // ------- слово не перевелось ---------
       ...
       Str1=Str ;
       s_text_yxf( y,x1+p1,c,Str1 );
       s_text_yxf( y,x1+p2,c,"?не знаю?" );
   }
   else
   {
       t_Variants  *VV =Core.variants( Begin+i, i_struct );
       ...
       s_text_yxf( y,x1+p1,c,Str1 );
       s_text_yxf( y,x1+p2,c,Grammar[i_struct].From.Name );

       sprint_param( Str2, 100, i_struct, VV1->Form, 0);
       s_text_yxf( y,x1+p3,c,Str2 );
   }
}
/***************************************************************************/
e_WinMsg t_FirstLook :: loop( void )
{
   short sx,sy ;
   ushort ZZ,key1,key2 ;
   ...
   while( 1 )
   {
	 ...
	 {   switch( key2 )
	     {
	        /*
		case S_key_Ins :         // должно быть другое действие
                {   short i,p1,p2 ;   
		    char  Str[200]="" ;
		...
		*/
		case S_key_Up   : step_up(); break ;
		case S_key_Down : step_dn(); break ;
		case S_key_Left : if( 0<Begin_X ) Begin_X-- ; break ;
		case S_key_Right : if( Begin_X<80 ) Begin_X++ ; break ;
		case S_key_Home : PY=0 ; break ;
		case S_key_End  : PY=MaxY-1 ; break ;
      ...
      if( PY<Begin ) Begin=PY ;
      if( size_y-1<PY-Begin ) Begin=PY-(size_y-1) ;
   }
}
/***************************************************************************/
void t_FirstLook :: step_up()
{
   if( PY>0 ) PY-- ;
   if( PY<Begin ) Begin=PY ;
}
/***************************************************************************/
void t_FirstLook :: step_dn()
{
   if( PY<MaxY-1 ) PY++ ;
   if( size_y-3<PY-Begin ) Begin=PY-(size_y-3) ;
}
/***************************************************************************/
void t_FirstLook :: set_py( short py )
{
   PY=py ;
   if( PY<Begin ) Begin=PY ;
   if( size_y-3<PY-Begin ) Begin=PY-(size_y-3) ;
...
/***************************************************************************/
short t_FirstLook :: get_py(  )
{
   return PY ;
}
/***************************************************************************/
char * t_FirstLook :: get_from( )
{
   t_ItemList &From =Core.from( );
   if( 0<=PY && PY<From.j )
            return From[PY].Str ;
...
/***************************************************************************/
char t_FirstLook :: hot_key( short SS, short key1, short key2 )
{
   switch( key1 )
   {  case S_key_Enter :
...
{  short  i,i1,j ;
   ...
   {  t=Grammar[i].From.type ;
      ...
      {  if( 99<=j ) break ;
         Tag[j].i_struct=i ;
         Tag[j].i_dic   =VV->Variant[i1].i_dic ;
...
{  short  i1,z ;
   ...
   s1=emptystr( size_x );
   s_text_yxf( y1+i+1,x1+1 ,c, s1 );

   // --------- начепятать имя структуры --------------
   if( MaxY<=Begin+i ) return ;
   strcpy( Str,emptystr(size_x-3) );
   ...
   if( Tag[Begin+i].i_dic==0 )
            Str1=Perevod->translate_word_i( Word, Tag[Begin+i].i_slowo,Tag[Begin+i].i_slowo1 );
       else Str1=Dogadka.translate_word_i( Word, Tag[Begin+i].i_slowo,Tag[Begin+i].i_slowo1 );
   ...
   switch( key1 )
   {
	  ...
	  {  t_Param *P1=&LParam->list[SParam->list[PY].param] ;
	     t_EditForm1 EditForm1( P1,&value[PY] );
			s_save( y1, x1, y2, x2 );
			s_rame2_F( y1, x1, y2, x2, 0x0f );
			s_text_yxf( y1, x1+3, titl_Color, "Задание параметра" );
...
void t_SlowoView :: paint_begin( void )
{
   n_From=Grammar.from().Format.j ;
   MaxY  =n_From+Grammar.to().Format.j ;
   s_text_yxf( y1-1,x1+3 ,titl_Color, "Грамматические словари" );
...
{  char *s,Str[200]="",Str1[50] ;
   ...
   s_text_yxf( y1-2,x1+2, titl_Color,Str );  
}
/***************************************************************************/
