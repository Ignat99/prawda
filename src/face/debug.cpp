...
//     t_Slowo3View  смотрелка для t_Slowo3 (главный словарь)
//     t_Slowo3View1 смотрелка для t_Slowo3 (словарь догадок)
...
/***************************************************************************/
//        нарисовать стек (лавое окошко - конструкции,варианты )
/***************************************************************************/
...
{  long i,z ;
   ...
   if( Reg==DSTRUCT ) // режим конструкции
   {  for( i=0 ; i<size_y-6 ; i++ )
      {  strcpy( Str,emptystr(79) );
	 long i_struct=Begin_S+i ;
	 if( (Reg1==D1STRUCT && i_struct<Grammar.n_trans()) ||
	     (Reg1==D1SELECT && i_struct<SelectVision.j ) )
	 {   
	     if( Reg1==D1SELECT ) i_struct=SelectVision[i_struct] ;
	     char try1='*' ;	     
	     VV=Core.variants( Current.i_word,i_struct );
             if( VV->absend==0 && VV->Variant.j==0 ) try1=' ' ;
	     sprintf( Str,"%4d %c %2ld %s",i_struct,try1,VV->Variant.j,Grammar[i_struct].From.Name );
	 }
         if( i_struct==z_Vision ) f=0x70 ; else f=0x07 ;
	 Str[xx-x1-3]=0 ;	 
...
/***************************************************************************/
void t_Debug :: paint_variant1( short x1, short x2 )
{  long i,i_variant ;
   char f ;
   char Str[180],Str1[100] ;

   t_Variants *VV=Core.variants( Current.i_word,Current.i_struct );
   for( i=0 ; i<size_y-7 ; i++ )
   {  strcpy( Str,emptystr(179) );
      Str[x2-x1-1]=0 ;
      i_variant=Begin_V+i ;      
      if( i_variant==Current.i_variant ) f=0x70 ; else f=0x07 ;
      s_text_yxf( y1+7+i, x1+1, f, Str );
      if( i_variant<VV->Variant.j )
      {   t_rStruct *V=&VV->Variant[i_variant] ;
          sprint_param( Str1, 100, Current.i_struct, V->Form, 0 );
          sprintf( Str,"%2ld %d-%d (%s)",i_variant,V->i_word,V->i_last_word,Str1 );
      }
      Str[x2-x1-1]=0 ;
      s_text_yxf( y1+7+i, x1+1, f, Str );
   }
...
{  long        i ;
   ...
   {   if( SS->type==TSTRUCT  ) s_text_yxf( y1+6, xx+4 , 0x0f, "Структура" );
	  ...
	  {   t_String Str3,Str4 ;
	      V1=Core.variant( R ) ;
	      sprint_param1( Str3, W->Param );
	      Str4=Str1 ;
	      Str4+=Str3 ;
	      s_text_yxf( y1+7+i, xx+4, c, Str4.c_str() );
   ...
   {   
	   ...
	   else sS=Dogadka.get_from( V->i_slowo );
...
//                   нарисовать параметры структуры
/***************************************************************************/
void t_Debug :: sprint_param1( t_String &Str, t_Param1List &Param )
{
   long  i1,i2,i3 ;   
   char *s1 ;
	 ...
	 {   switch( key2 )
		...
		case S_key_F8 :
		    {  short yy1,yy2,xx1,xx2 ;
		       ...
		       s_text_yxf( yy1, xx1+3, titl_Color, "Внутренняя передача параметров" );
...
/********************************************************************/
//       смотрелка для t_Slowo3 словарь догадок                     //
/********************************************************************/
t_Slowo3View1 :: t_Slowo3View1( void ) : t_SelectWin()
{  
   strcpy( Name,"Словарь догадок" );
}
/********************************************************************/
void t_Slowo3View1 :: paint1( short i, char current )
{  short  xx ;
   char   c ;
   char  *Str ;
   t_String STmp ;
   ...
   s_text_yxf( y1+i,x1+1 ,c, Str );
   if( Dogadka.n_record()<=Begin+i ) return ;
   Dogadka.print_record( STmp, Begin+i );
   s_text_yxf( y1+i,x1+1 ,c, (char *)STmp.c_str() );
}
/********************************************************************/

/********************************************************************/
t_Core1View :: t_Core1View( void )
{  strcpy( Name,"Дерево словообразования" );
   Vision=NULL ;
}
/********************************************************************/
void t_Core1View :: paint_begin( void )
{  long   i,i1,j ;
   t_mTreeVsion V ;

   MaxY=Core1.Tree.j ;
   for( i=0 ; i<Core1.Tree.j ; i++ )
      MaxY+=Core1.Tree[i].i_last_dogadka-Core1.Tree[i].i_dogadka ;
   Free( Vision );
   Vision=(t_mTreeVsion *)Calloc( MaxY,sizeof(t_mTreeVsion) );

   for( i=j=0 ; i<Core1.Tree.j ; i++ )
   {  V.i_tree   = i ;
      V.i_dogadka=-1 ;
      Vision[j++]=V ;
      for( i1=Core1.Tree[i].i_dogadka ; i1<Core1.Tree[i].i_last_dogadka ; i1++ )
      {  V.i_tree   =-1 ;
         V.i_dogadka=i1 ;
         Vision[j++]=V ;
      }
   ...
   i=strlen( Core1.Str0 );
   s_text_yxf( y1-1,x1+38   ,0x0f, "[ " );
   s_text_yxf( y1-1,x1+30   ,0x0f, Core1.Str0 );
   s_text_yxf( y1-1,x1+30+i ,0x0f, " ]" );   
}
/********************************************************************/
void t_Core1View :: paint1( short i, char current )
{  char  c ;
   char *Str,*s,*s1 ;
   t_mTreeVsion *V ;
   ...
   V=&Vision[Begin+i] ;
   if( 0<=V->i_tree )
   {   t_mTree *T=&Core1.Tree[V->i_tree] ;

       if( 0==T->type ) s="Синт" ;
           else         s="Анал" ;
       s1=Grammar.Step[T->type_link].Name ;
       sprintf( Str," %d %d %s %s  %s",V->i_tree,T->up,s,s1,T->Str );
       s_text_yxf( y1+i,x1+1 ,c, Str );
   }
   if( 0<=V->i_dogadka )
   {   t_String STmp ;

       Dogadka.print_record( STmp, V->i_dogadka );
       s_text_yxf( y1+i,x1+5 ,c, (char *)STmp.c_str() );
   }
}
/********************************************************************/
char t_Core1View :: hot_key( short SS, short key1, short key2 )
{
   if( 0==key1 && S_key_Ins==key2 )
   {   char  Str[50]="" ;
       t_longList Antwort ;
       e_WinMsg r ;
       ...
       s_rame1_f( y3-1, x3-31, y3+1, x3+31, 0x07 );
       s_text_yxf( y3-1, x3-27, 0x07, "Переводимое слово" );
       r=kommand_yxf( y3, x3-30, 0x07, Str, 50, 0 );
       if( WM_OK!=r ) 
           return 0 ;
       Core1.make_dogadka( Str, Antwort );
       return 1 ; 
   }
   return 0 ;
}
