...
{
   return strcmp( ((t_Gender *)A)->Word,((t_Gender *)B)->Word );
}
/***************************************************************************/
...
int main1( char *FileGrammar, char *FileSrc, char *FileDst )
{
   ...
   if( r<0 ) return r ;
   myPerevod.read( FileSrc,&Grammar.format1()[0] );
   ...
   for( i=0 ; i<myPerevod.n_record() ; i++ )
   {  N1=myPerevod.get_n_perevod( i );
      for( i1=0 ; i1<N1 ; i1++ )
      {  
         t_sStruct *S=myPerevod.get_to( i,i1 );
	 ...
	 t_Gender W ;
	 W.Word=S->Word[0].str ;
	 i_word=Qfind( Word,n_Word,sizeof(t_Gender),wordcmp3,&W );
...
int main( void )
{
   ...
   n_Word=N/3 ;
   Word=(t_Gender *)Calloc( n_Word,sizeof(t_Gender) );
   for( i=0 ; i<n_Word ; i++ )
   {  Word[i].Word  =Str[3*i] ;
      Word[i].Gender=Str[3*i+1] ;
   }
   // --------- отсортировать его ------------------
   qsort( Word,n_Word,sizeof(t_Gender),wordcmp3 );
