...
{  @Структура фраза_тот_что = 
              _запятая @0[what] фраза ;
   @Структура фраза_тот_что( падеж &Падеж ) = 
              _запятая мест_прил[тот]( %С, Падеж, %Ед ) @0[что] фраза ;
}
@Перевод
{  @Структура фраза_который = 
              _запятая @0[that] сказуемое( &S1 ) хвост ;
   @Структура фраза_который( род &Род, падеж &Падеж, число &Число ) =
              _запятая мест_прил[который]( Род, Падеж, Число ) 
	      сказуемое( %3е, Число, Род ) хвост ;
}
@Перевод
{  @Структура фраза_be_который =
              _запятая @0[that] сказуемое_be хвост_be_a ;
